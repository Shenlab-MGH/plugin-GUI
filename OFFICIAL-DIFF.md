...
|---|---|---|---|---|---|---|
| JuceLibraryCode/modules/juce_gui_basics/native/accessibility/juce_AccessibilityElement_windows.cpp | MODIFIED | JUCE accessibility element behavior and automatic Window/Transform patterns on fragment roots | Preserves stable Windows automation IDs and suppresses Window/Transform providers only for the explicitly identified Agent-restricted main window | Accessibility-only Windows behavior or accidental Close/resize exposure | Full-PID mutable-pattern verifier, source contract, and Release build | Build official JUCE baseline |
...
| Source/MainWindow.cpp | MODIFIED | Official window lifecycle; a loaded graph is normally written to recoveryConfig only after later edits or shutdown | Starts/stops an optional narrow Agent UIA bridge; the editor and window-chrome subtrees stay hidden and the boundary is reapplied after LookAndFeel rebuilds; a separate explicit interactive flag adds only two semantic bridge actions; mutation requires explicit runtime flag plus process approval ID; after a successful graph load, only an explicitly isolated state directory receives an immediate runtime recovery graph for live attestation | Window lifecycle, unintended UI control exposure, authorization, or stale/missing runtime graph evidence | UIA full-PID action allowlist, loopback, runtime-option, launcher, exact-mode, executable-path, independent executable-hash, runtime graph source contract, and live state-restore acceptance | Omit Agent flags and state-directory isolation |
| Source/MainWindow.h | MODIFIED | Official window lifecycle | Owns optional Agent components, declares the repeated non-bridge accessibility boundary, and declares isolated runtime graph publication | Window lifecycle, unintended window action exposure, or graph-evidence lifecycle | UIA full-PID action allowlist, runtime graph source contract, and loopback live acceptance | Agent mode off and omit state-directory isolation |
