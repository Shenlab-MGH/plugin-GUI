...
| Source/MainWindow.cpp | MODIFIED | Official window lifecycle | Starts/stops optional Agent endpoint and UIA bridge; mutation requires explicit runtime flag plus process approval ID | Window lifecycle and authorization | UIA, loopback, runtime-option, and launcher acceptance | Omit Agent flags |
...
| Source/UI/ControlPanel.cpp | MODIFIED | Official acquisition/record buttons and filename fields | Routes semantic origins, adds exact directory preparation, shares recording safety detection, blocks Agent sync override, and clears stale stored prepend/append values when selecting none | Control, safety, and naming parity | command routing, recording safety, directory adapter contract, and GUI acceptance | Build official tag |
