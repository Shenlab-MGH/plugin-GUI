...
|---|---|---|---|---|---|---|
| JuceLibraryCode/modules/juce_gui_basics/native/accessibility/juce_AccessibilityElement_windows.cpp | MODIFIED | JUCE derives Windows AutomationIds primarily from accessible titles | Preserves explicit component IDs as stable Windows AutomationIds; the ordinary top-level HWND continues to expose its operating-system Window/Transform patterns | Accessibility-only Windows identity behavior or AutomationId collisions | Exact-ID source contract, full-PID Invoke/Toggle/Window/Transform inventory, and Release build | Build official JUCE baseline |
...
| Source/Agent/AgentAccessibilityAction.cpp | ADDED | No Agent accessibility action planner | Maps only Acquisition and Recording press actions to explicit target modes and rejects unknown state | Incorrect target transition | AgentAccessibilityActionTests and live UIA state readback | Interactive UIA flag off |
| Source/Agent/AgentAccessibilityAction.h | ADDED | No Agent accessibility action planner | Declares the allowlisted fail-closed target mapping | Incorrect target transition | AgentAccessibilityActionTests | Interactive UIA flag off |
| Source/Agent/AgentAccessibilityBridge.cpp | ADDED | No Agent accessibility bridge | Read-only semantic UIA tree by default; explicit interactive mode adds only Acquisition and Recording press actions through deadline-bounded verified transport requests, exposes correlation and terminal state, and logs submission plus final outcome | Additional UI action surface, delayed action, and incomplete audit | UIA source contract plus live full-PID Invoke/Toggle/Window/Transform allowlist, request-terminal, audit-log, and state-restore acceptance | Agent UIA flag off |
| Source/Agent/AgentAccessibilityBridge.h | ADDED | No Agent accessibility bridge | Declares the narrow read-only or explicit interactive UIA bridge | Additional UI surface | UIA source contract | Agent UIA flag off |
...
| Source/Agent/AgentTransportCoordinator.cpp | ADDED | No Agent transport coordinator | Applies typed state plans with readback, rechecks deadlines before every start step, and preserves expired failsafe stop steps | Incorrect transition, late start, or blocked emergency stop | AgentTransportCoordinatorTests including expired-start rejection and expired-stop completion | Mutation flag off |
...
| Source/Agent/AgentTransportEndpoint.cpp | ADDED | No Agent endpoint | Schedules endpoint work on GUI thread and rejects expired deadline-bounded requests before execution | Thread/lifecycle and delayed-action risk | AgentTransportEndpointTests including expired-request non-execution | Agent mode off |
...
| Source/Agent/AgentTransportPlanner.h | ADDED | No Agent transition planner | Declares planner contract and optional monotonic execution deadline | Unsafe plan or delayed action | AgentTransportPlannerTests and AgentTransportEndpointTests | Mutation flag off |
...
| Source/Agent/RuntimeOptions.cpp | ADDED | No Agent flags | Parses fail-closed Agent isolation and explicit mutation flags | Isolation or authorization bypass | RuntimeOptionsTests | Omit Agent flags |
| Source/Agent/RuntimeOptions.h | ADDED | No Agent flags | Declares isolation and default-off mutation options | Isolation or authorization bypass | RuntimeOptionsTests | Omit Agent flags |
...
| Source/MainWindow.cpp | MODIFIED | Official window lifecycle; a loaded graph is normally written to recoveryConfig only after later edits or shutdown | Starts/stops an optional narrow Agent UIA bridge; the native editor subtree is hidden while the standard top-level HWND remains visible to Windows UIA; a separate explicit interactive flag adds only Acquisition and Recording Invoke actions; mutation requires explicit runtime flag plus process approval ID; after a successful graph load, only an explicitly isolated state directory receives an immediate runtime recovery graph for live attestation | Window lifecycle, OS-level close/move/resize exposure, authorization, or stale/missing runtime graph evidence | Full-PID Invoke/Toggle/Window/Transform inventory, loopback, runtime-option, launcher, exact-mode, executable-path, independent executable-hash, runtime graph source contract, and live state-restore acceptance | Omit Agent flags and state-directory isolation |
| Source/MainWindow.h | MODIFIED | Official window lifecycle | Owns optional Agent components, declares the non-bridge editor accessibility boundary, and declares isolated runtime graph publication | Window lifecycle, OS-level window action exposure, or graph-evidence lifecycle | Full-PID Invoke/Toggle/Window/Transform inventory, runtime graph source contract, and loopback live acceptance | Agent mode off and omit state-directory isolation |
...
| Source/UI/ControlPanel.cpp | MODIFIED | Official acquisition/record buttons and filename fields | Routes semantic origins, adds exact directory preparation, shares recording safety detection, blocks Agent sync override, clears stale stored prepend/append values when selecting none, keeps native transport controls inaccessible to UIA, and rechecks active deadlines immediately before acquisition or recording start | Control, safety, naming parity, late start, and unintended UI control exposure | command routing, recording safety, deadline source contract, directory adapter contract, and GUI acceptance | Build official tag |
