...
| Source/MainWindow.cpp | MODIFIED | Official window lifecycle | Starts/stops an optional narrow Agent UIA bridge; the editor and window-chrome subtrees stay hidden and the boundary is reapplied after LookAndFeel rebuilds; a separate explicit interactive flag adds only two semantic bridge actions; mutation requires explicit runtime flag plus process approval ID | Window lifecycle, unintended UI control exposure, and authorization | UIA full-PID action allowlist, loopback, runtime-option, launcher, exact-mode, executable-path, independent executable-hash, and live state-restore acceptance | Omit Agent flags |
| Source/MainWindow.h | MODIFIED | Official window lifecycle | Owns optional Agent components and declares the repeated non-bridge accessibility boundary | Window lifecycle and unintended window action exposure | UIA full-PID action allowlist and loopback live acceptance | Agent mode off |
