...
from Tests.CI.test_official_deploy_guards import (
    _mapping_block,
    _sequence_values,
    workflow_steps_from_text,
)
...
ALLOWED_RELEASES = {"v1.0.2-r0.1.0", "v1.1.0-r0.1.0"}
CONTRACT_COMMAND = (
...
class AgentWindowsPortablePackagerTests(unittest.TestCase):
    ...
    def _run_packager(
        self, root: Path, gui_version: str, release_version: str
    ...
    def test_packages_v110_with_deterministic_name_checksum_and_provenance(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            result = self._run_packager(root, "1.1.0", "v1.1.0-r0.1.0")
    ...
    def test_packages_v102_with_its_own_baseline_identity(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            result = self._run_packager(root, "1.0.2", "v1.0.2-r0.1.0")
    ...
    def test_rejects_release_when_checked_out_gui_baseline_does_not_match(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            result = self._run_packager(root, "1.1.0", "v1.0.2-r0.1.0")
    ...
    def test_rejects_unversioned_or_unapproved_release_names(self) -> None:
        for release_version in ("latest", "v1.1.0-r0.2.0", "v1.1.0"):
            with self.subTest(release_version=release_version):
                with tempfile.TemporaryDirectory() as directory:
                    result = self._run_packager(
                        Path(directory), "1.1.0", release_version
                    )
                    self.assertNotEqual(result.returncode, 0)
...
class AgentWindowsPortableWorkflowTests(unittest.TestCase):
    ...
    def test_release_workflow_has_only_exact_tag_and_dispatch_triggers(self) -> None:
        ...
        self.assertEqual(_sequence_values(pushes, "tags", 4), ALLOWED_RELEASES)
        self.assertNotIn("branches:", pushes)
        self.assertEqual(_sequence_values(dispatch, "options", 8), ALLOWED_RELEASES)
    ...
    def test_build_is_windows_release_portable_and_uploads_exact_bundle(self) -> None:
        ...
        steps = workflow_steps_from_text(self.release_source)
        upload_steps = [
            fields
            for fields, _ in steps
            if fields.get("uses") == ["actions/upload-artifact@v4"]
        ]
        self.assertEqual(len(upload_steps), 1)
        self.assertIn("steps.release.outputs.artifact_name", self.release_source)
        self.assertIn("dist/${{ steps.release.outputs.artifact_name }}", self.release_source)
    ...
    def test_publish_job_is_limited_to_matching_shenlab_tag_and_existing_prerelease(self) -> None:
        ...
        self.assertIn("needs.build.outputs.release_version == github.ref_name", publish_job)
