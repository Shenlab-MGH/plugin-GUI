...
{
public:
    ~ComOwner() { if (value != nullptr) value->Release(); }
...
UiaObservation observeProcessorInventory (HWND windowHandle)
{
    ...
    ComOwner<IUIAutomationElementArray> children;
    if (SUCCEEDED (observation.result))
        observation.result = root->FindAll (TreeScope_Children, trueCondition.get(), children.put());
    int count = 0;
    if (SUCCEEDED (observation.result) && children.get() != nullptr)
        observation.result = children->get_Length (&count);
    observation.directChildCount = count;
    if (SUCCEEDED (observation.result) && count > 0)
    {
        observation.directChildrenAreOnlyProcessorItems = true;
        for (int index = 0; index < count; ++index)
        {
            ComOwner<IUIAutomationElement> child;
            BSTR id = nullptr;
            CONTROLTYPEID type = 0;
            if (FAILED (children->GetElement (index, child.put()))
                || FAILED (child->get_CurrentAutomationId (&id))
                || FAILED (child->get_CurrentControlType (&type)))
            {
                SysFreeString (id);
            ...
            BSTR name = nullptr;
            if (SUCCEEDED (child->get_CurrentName (&name)) && name != nullptr)
                itemObservation.name = String (name);
            SysFreeString (name);

            VARIANT fullDescription;
            VariantInit (&fullDescription);
            if (SUCCEEDED (child->GetCurrentPropertyValue (
            ...
            BOOL isOffscreen = FALSE;
            if (SUCCEEDED (child->get_CurrentIsOffscreen (&isOffscreen)))
                itemObservation.isOffscreen = isOffscreen != FALSE;
            ComOwner<IUnknown> invoke;
            if (SUCCEEDED (child->GetCurrentPattern (UIA_InvokePatternId, invoke.put())))
    ...
    if (SUCCEEDED (observation.result))
        observation.result = findByAutomationId (automation.get(), root.get(), L"oe.processor.4242",
                                                 TreeScope_Children, item.put());
    if (SUCCEEDED (observation.result) && item.get() != nullptr)
    {
...
UiaObservation observeWhilePumpingMessages (HWND windowHandle)
{
    ...
    EXPECT_TRUE (finished.load()) << "Timed out waiting for the external COM client";
    while (! finished.load())
        MessageManager::getInstance()->runDispatchLoopUntil (10);
    worker.join();
    return observation;
...
TEST_F (ProcessorInventoryWindowsUIAutomationTests, ExternalClientSeesAReadOnlyListWithOnlyProcessorItems)
{
