...
CAPABILITY_IDS = [
    ...
    "oe.control.signal_chain.configuration", "oe.control.signal_chain.processors",
    "oe.status.cpu_usage",
...
class McpV004Tests(unittest.TestCase):
    ...
    def test_directory_uses_one_field_pre_put_post_readback_and_refuses_record(self):
        self.ready_server()
    ...
    def test_processor_inventory_validates_full_raw_schema_then_returns_only_closed_projection(self):
        self.ready_server()
        self.api.processors = {"processors": [
            {"id": 11, "name": "File Reader",
             "parameters": [{"name": "path", "type": "String", "value": "C:/data"}],
             "streams": [{"name": "Neuropixels AP", "source_id": 11, "sample_rate": 30000.0,
                          "channel_count": 384, "parameters": [{"name": "gain", "type": "Float", "value": "500"}]}],
             "predecessor": None},
            {"id": 22, "name": "Mouse probe filter", "parameters": [], "streams": [], "predecessor": 11},
        ]}
        result, payload = self.call("oe_get_processors")
        self.assertNotIn("isError", result)
    ...
    def test_processor_inventory_is_no_args_and_rejects_schema_drift(self):
        self.ready_server()
        self.assert_error("oe_get_processors", {"id": 22}, "invalid_arguments")
        valid_parameter = {"name": "gain", "type": "Float", "value": "500"}
        valid_stream = {"name": "Neuropixels AP", "source_id": 1, "sample_rate": 30000.0,
                        "channel_count": 384, "parameters": [valid_parameter]}
        valid_processor = {"id": 1, "name": "Probe", "parameters": [valid_parameter],
                           "streams": [valid_stream], "predecessor": None}
        invalid_payloads = (
            {}, {"processors": "not-a-list"}, {"processors": [], "extra": True},
            {"processors": [{key: value for key, value in valid_processor.items() if key != "streams"}]},
            {"processors": [{**valid_processor, "id": True}]},
            {"processors": [{**valid_processor, "name": 1}]},
            {"processors": [{**valid_processor, "parameters": [{"name": "gain", "type": "Float"}]}]},
            {"processors": [{**valid_processor, "parameters": [{"name": "gain", "type": "Float", "value": 500}]}]},
            {"processors": [{**valid_processor, "streams": [{**valid_stream, "source_id": True}]}]},
            {"processors": [{**valid_processor, "streams": [{**valid_stream, "name": 1}]}]},
            {"processors": [{**valid_processor, "streams": [{**valid_stream, "channel_count": True}]}]},
            {"processors": [{**valid_processor, "predecessor": True}]},
            {"processors": [valid_processor, {**valid_processor, "name": "Duplicate"}]},
        )
        for payload_value in invalid_payloads:
            with self.subTest(payload=payload_value):
                self.api.processors = payload_value
                self.assert_error("oe_get_processors", {}, "response_schema_mismatch")
        with self.assertRaises(self.module.ToolError) as nonfinite:
            self.module.processors_projection({"processors": [{**valid_processor, "streams": [{**valid_stream, "sample_rate": math.nan}]}]})
        self.assertEqual(nonfinite.exception.code, "response_schema_mismatch")

    def test_empty_processor_inventory_is_valid(self):
        self.ready_server()
        self.api.processors = {"processors": []}
        self.assertEqual(self.call("oe_get_processors")[1], {"processors": []})
