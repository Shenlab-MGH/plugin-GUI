...
description: Safely inspect and control Open Ephys v1.1.0 through the additive 0.0.5 MCP tools, including fail-closed Neuropixels preset inventory and selection without GUI actions.
---
...
- compatibility spelling: hardware_verified:false
- compatibility spelling: scientific_verified:false
...
2. Read current state before proposing a mutation:
   ...
   - Call `oe_get_electrode_presets` only when the exact preset capability is present.
...
9. Change a preset only in IDLE with `oe_set_electrode_preset`, using the exact
   processor, probe, serial, generation, preset ID, and electrode-map hash from
   a fresh inventory. Never retry an unknown mutation outcome or fall back to GUI clicks.
...
- `oe_get_electrode_presets`: read exact preset inventory and selected state.
- `oe_set_electrode_preset`: perform one IDLE-only compare-and-set with fresh readback.
...
Zero compatible processors or a preset-version mismatch is
`capability_unavailable`. Multiple compatible processors is `ambiguous_target`
for both GET and PUT, and the PUT must not dispatch a plugin mutation.

Historical compatibility remains available under contract: 0.0.4. Do not
assume tools, controls, or workflows outside the listed 0.0.5 surface.
