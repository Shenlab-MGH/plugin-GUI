...
#ifndef CONFIG_SNAPSHOT_API_HANDLER_H
#define CONFIG_SNAPSHOT_API_HANDLER_H
...
template <typename Dispatcher, typename ReadSnapshot>
void handleConfigSnapshotGet (const httplib::Request&,
                              ...
                              std::chrono::milliseconds timeout)
{
    const auto readResult = handleControlRead (
        ...
        std::forward<ReadSnapshot> (readSnapshot),
        timeout);
    ...
    if (! readResult.value.has_value())
    {
        ...
        document["ok"] = false;
        document["capability"] = "oe.control.signal_chain.configuration";
    ...
    nlohmann::json document;
    document["info"] = readResult.value->toStdString();
    response.status = 200;
    response.set_content (document.dump(), "application/json");
}
